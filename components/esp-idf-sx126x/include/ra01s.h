...
typedef struct sx126x_config_s {
...
typedef struct sx126x_context_t sx126x_context_t;

typedef struct sx126x_context_t *sx126x_handle_t;
...
bool      LoRaSendAsync(sx126x_handle_t handle, uint8_t *pData, int16_t len);
bool      LoRaTxWaitDone(sx126x_handle_t handle, uint16_t *irq_status_out);
...
void SetDioIrqParams(sx126x_handle_t handle, uint16_t irqMask, uint16_t dio1Mask, uint16_t dio2Mask, uint16_t dio3Mask);
...
int      GetPacketLost(sx126x_handle_t handle);
...
void     Wakeup(sx126x_handle_t handle);
...
void SPItransfer(sx126x_handle_t handle, uint8_t cmd, bool write, uint8_t *dataOut, uint8_t *dataIn, uint8_t numBytes,
                 bool waitForBusy);
